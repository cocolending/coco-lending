...
contract ComptrollerG6 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function setDistributor(ICocoDistributor distributor) external {
    	require(msg.sender == admin, "only admin can set Distributor");
        require(distributor.comptroller() == address(this), "invalid distributor");
        cocoDistributor = distributor;
    }
    // ------
    function distributeSupplierComp(address cToken, address supplier, bool distributeAll) internal {
        if(address(cocoDistributor) != address(0))
            cocoDistributor.distributeSupplierComp(cToken, supplier, distributeAll);
    }
    function updateCompSupplyIndex(address cToken) internal {
        if(address(cocoDistributor) != address(0))
            cocoDistributor.updateCompSupplyIndex(cToken);
    }
    function distributeBorrowerComp(address cToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        if(address(cocoDistributor) != address(0))
            cocoDistributor.distributeBorrowerComp(cToken, borrower, marketBorrowIndex.mantissa, distributeAll);
    }
    function updateCompBorrowIndex(address cToken, Exp memory marketBorrowIndex) internal {
        if(address(cocoDistributor) != address(0))
            cocoDistributor.updateCompBorrowIndex(cToken, marketBorrowIndex.mantissa);
    }
}
